...
async function calculateDepthMap(normalMap) {
   ...
   const depthMapping = await new Promise(async (resolve) => {
      const anglesCount = azimuthalAngles.length;

      let promisesResolvedCount = 0;
      let integralArrayLock = false;
      const integralArray = new Array(normalMap.width * normalMap.height).fill(
         0
      );
      ...
      for (let i = 0; i < anglesCount; i++) {
         ...
         const integralPromise = new Promise((resolve) => {
            ...
            azimuthalAngle += 180; // Inverse and thus, line FROM and NOT TO azimuthal angle.
            const azimuthalAngleRadians = azimuthalAngle * (Math.PI / 180);

            const stepVector = {
               x: Math.cos(azimuthalAngleRadians),
               y: Math.sin(azimuthalAngleRadians),
            };
      ...
      console.log(normalizedIntegral);
